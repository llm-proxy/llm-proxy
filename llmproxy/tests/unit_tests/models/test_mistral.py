...
def test_mistral_invalid_api_key() -> None:
    ...
    # Assert
    assert check.message == "ERROR: Authorization header is correct, but the token seems invalid" 
...
def test_mistral_temp_under_0() -> None:
    ...
    print(check.payload)
    # Assert
    assert check.message == "ERROR: Input validation error: `temperature` must be strictly positive"