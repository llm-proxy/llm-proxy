...
class LLMProxy:
    ...
    def route(
        ...
        match RouteType(self.route_type):
            case RouteType.COST:
                return self._cost_route(prompt=prompt, chat_history=self.chat_history)
            case RouteType.CATEGORY:
                return self._category_route(
                    prompt=prompt, chat_history=self.chat_history
                )
            case RouteType.ELO:
                return self._elo_route(prompt=prompt, chat_history=self.chat_history)
            case _:
    ...
    def _cost_route(self, prompt: str, chat_history: List[Dict[str, str]]):
        ...
        while not completion_res:
            ...
            try:
                completion_res = self._make_provider_call(
                    prompt=prompt,
    ...
    def _make_provider_call(
        self, prompt: str, chat_history: List[Dict[str, str]], instance_data: List[Any]
    ) -> str:
        """
        Make a call to the model to retrieve a response, suppressing any additional logging
        generated internally within the clients.

        Args:
            prompt (str): The prompt to be used for generating the response.
            instance_data (List[Any]): A list containing information about the provider instance.

        Returns:
            str: The response generated by the model.
        """
        import logging
        from proxyllm.utils.logger import NullHandler

        completion_res = None

        # Temporarily attach the NullHandler to the root logger
